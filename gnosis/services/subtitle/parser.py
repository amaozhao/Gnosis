...
class Subtitle:
    ...
    def _clean_content(self, content):
        ...
        legal_content_lines = [line for line in stripped_lines if line]  # 修正：if line
...
class SRTParseError(Exception):
    ...
    def __init__(self, expected_start, actual_start, unmatched_content):
        message = (
            f"Expected contiguous start of match or end of input at char {expected_start}, "
            f"but started at char {actual_start} (unmatched content: {unmatched_content!r})"
        )
...
class SrtParser:
    ...
    def compose(
        ...
        return "".join(
            subtitle.to_srt(strict=strict, eol=eol) for subtitle in subtitles_to_compose
        )
